...
void Dialog_Stats::statistiques(QVector<double>* ticks,QVector<QString> *labels)
{
    QSqlQuery qry;
    int i=0;
    qry.exec("SELECT REFFERANCE FROM EQUIPEMENT");
    while (qry.next())
    {
        QString REFERANCE = qry.value(0).toString();
        i++;
        *ticks<<i;
        *labels <<REFERANCE;
    }
...
void Dialog_Stats::MakeStat()
{
    ...
    A->setName("Les Montants des equipements selon les Refferances");
    ...
    ui->plot->xAxis->setLabel("REFFERANCE");
    ...
    QSqlQuery q1("SELECT QUANTITE FROM EQUIPEMENT");
    while (q1.next())
    {
